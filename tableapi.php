...
namespace Tqdev\PhpCrudApi {
    ...
    $dbX = new MeekroDB($S['db']['host'], $S['db']['user'], $S['db']['password'], $S['db']['name']);

//    $_SESSION['dump'] = $dbX->tableList();
    $disallowed = [];
    if($S['security']['use_authentification']){
            if(isset($_SESSION['auth_logged_in'])){
                if($_SESSION['auth_logged_in'] == 1){

                    if($_SESSION['auth_roles'] != 1)  $disallowed =  ['users_throttling','users_resets','users_remembered','users_otps','users_confirmations','users_2fa','users'];

                }else{

                    $disallowed =  $dbX->tableList(); //authentification enabled but not logged in
    ...
    $config = new Config([
        ...
        'jwtAuth.secrets' => "xxxx",
        'jwtAuth.mode' => 'optional',    
        'jwtAuth.algorithms' => 'RS256', 
            'jwtAuth.leeway' => '60',
            'jwtAuth.header' => 'X-Authorization',

            'authorization.tableHandler' => function ($operation, $tableName) {
                global $disallowed;
                $restrictedList = $disallowed;
                //print_r($_SESSION);
                return !(in_array($tableName,$restrictedList));   
            },
